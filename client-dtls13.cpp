...
int main (int argc, char** argv)
{
    ...
    if (wolfSSL_CTX_load_verify_locations(ctx, servCertLoc, 0)
	    != SSL_SUCCESS) {
    ...
    if (wolfSSL_CTX_use_certificate_file(ctx, clientCertLoc, SSL_FILETYPE_PEM) != 
                                                                 SSL_SUCCESS) {
        fprintf(stderr, "Error loading %s, please check the file.\n", clientCertLoc);
        goto cleanup;
    }
    /* Load client Keys */
    if (wolfSSL_CTX_use_PrivateKey_file(ctx, clientKeyLoc,
